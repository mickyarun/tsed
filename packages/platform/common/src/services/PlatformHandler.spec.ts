...
describe("PlatformHandler", () => {
  ...
  describe("createHandler()", () => {
    it("should create handler", async () => {
      ...
      const result = service.createHandler(handler, metadata);

      expect(result).toBeInstanceOf(Function);

      await new Promise((resolve) => {
        $ctx.next = resolve;
        $ctx.runInContext(() => result($ctx));
      });

      expect(handler).toHaveBeenCalledWith({$ctx: null, args: undefined, status: "RESOLVED"});
    });
  });
